# get ptt beauty photos using asyncio
...
async def get_web_page_async(title, url, session):
...
async def get_beauty(loop, num, threshold=0, date=None):
    pool = mp.Pool()
    num_str = str(num)
    ...
    if page:
        ...
        #     print(post)
        #     if post['push_count'] >= threshold:
        #         post_page = get_web_page(ptt_url + post['href'])
        #         photo_links = get_photo_links(post_page)
        #         print(photo_links)
        #         save_urls(photo_links, post['title'], os.path.join("photos", num_str))
...
if __name__ == '__main__':
    ...
    for i in range(index_num - index_range, index_num):
        loop.run_until_complete(get_beauty(loop, i, threshold))
    date = time.strftime("%m/%d").lstrip('0')
    loop.run_until_complete(get_beauty(loop, index_num, threshold, date))
